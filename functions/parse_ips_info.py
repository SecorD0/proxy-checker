...
from utils.db_api.models import Proxy, IPInfo
        ...
        try:
            async with sem:
                ...
                async with aiohttp.ClientSession(connector=connector) as session:
                    async with session.get(url='https://ipapi.co/json', timeout=settings.timeout) as response:
...
async def parse_ips_info(settings: Settings) -> None:
    proxy_list = []
    for proxy in get_proxies():
        if not db.one(IPInfo, IPInfo.ip == proxy.ip):
        ...
        for proxy_list in proxy_lists:
            ...
            for response in await responses:
                try:
                    ...
                    db.insert(IPInfo(json_data=json_data))
                    if 'error' in json_data:
                        await cprint(text=f'Failed to parse IP info: {json_data["error"]}', color=Fore.RED)

                    else:
                        await cprint(text='IP info was parsed.', color=Fore.GREEN, proxy=proxy)

                except BaseException as e:
                    logging.exception('parse_ips_info')
                    await cprint(text=f'Failed to parse IP info: {e}', color=Fore.RED)
